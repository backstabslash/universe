...
class ConnectionHandler {
  async joinAndSendChannels(socket: Socket) {
    try {
      ...
      const userChannels = await ChannelUser.find({
      ...
      const dmsWithUsers = await ChannelUser.find({
        channel: { $in: dms },
      }).populate({
        path: 'user',
      ...
      for (const dmWithUser of dmsWithUsers) {
        const alreadyInFilteredDm = filteredDms.find(
          (filteredDm) =>
            filteredDm.channel.toString() === dmWithUser.channel.toString()
        );
        if (alreadyInFilteredDm) continue;
        const dublicateDm = dmsWithUsers.find(
          (dm) =>
            dmWithUser.channel.toString() === dm.channel.toString() &&
            dmWithUser.id !== dm.id
        );
      ...
      const dmsForLastMessage = filteredDms.map(
        (dmsWithUser) => dmsWithUser.channel
      );
      ...
      const latestChannelIds = latestMessages.map((message) =>
      ...
      const dmsIds = filteredDms.map((dmsWithUser) =>
        dmsWithUser.channel.toString()
      );

      const notesChannel = filteredDms.find(
        (dm) => dm.user.id === socket.data.userId
      );

      socket.join(allChannelIds.concat(dmsIds));
      socket.emit('send-channels', {
        channelGroups,
        dmsWithUsers: filteredDms.filter(
          (dm) => dm.user.id !== socket.data.userId
        ),
        notesChannel: {
